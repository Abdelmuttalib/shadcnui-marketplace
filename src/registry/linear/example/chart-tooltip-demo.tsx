...
export default function Component() {
  return (
    <div className="grid aspect-video w-full max-w-md justify-center text-linear-foreground md:grid-cols-2 [&>div]:relative [&>div]:flex [&>div]:h-[137px] [&>div]:w-[224px] [&>div]:items-center [&>div]:justify-center [&>div]:p-4">
      <div>
...
function TooltipDemo({
  ...
  return (
    <div
      className={cn(
        "grid min-w-[8rem] items-start gap-1.5 rounded-linear-lg border border-linear-border/50 bg-linear-background px-2.5 py-1.5 text-xs shadow-xl transition-all ease-in-out hover:-translate-y-0.5",
        className
      )}
    >
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={cn(
                "flex w-full items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-linear-muted-foreground",
            ...
            >
              <>
                ...
                >
                  ...
                  <span className="font-mono font-medium tabular-nums text-linear-foreground">
