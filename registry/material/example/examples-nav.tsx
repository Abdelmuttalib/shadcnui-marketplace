...
import { ScrollArea, ScrollBar } from "@/registry/material/ui/scroll-area";
        ...
        >
          {examples.map((example, index) => (
            <Link
              ...
              key={example.href}
              className={cn(
                "flex h-7 items-center justify-center rounded-material-md px-4 text-center text-sm transition-colors hover:bg-material-accent/80 hover:text-ring",
                pathname?.startsWith(example.href) ||
                  (index === 0 && pathname === "/")
                  ? "bg-material-accent font-medium text-ring"
