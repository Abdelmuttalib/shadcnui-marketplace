...
import { ScrollArea, ScrollBar } from "@/registry/catalyst/ui/scroll-area";
        ...
        >
          {examples.map((example, index) => (
            <Link
              ...
              key={example.href}
              className={cn(
                "flex h-7 items-center justify-center rounded-md px-4 text-center text-sm transition-colors hover:bg-catalyst-accent/80 hover:text-ring",
                pathname?.startsWith(example.href) ||
                  (index === 0 && pathname === "/")
                  ? "bg-catalyst-accent font-medium text-ring"
