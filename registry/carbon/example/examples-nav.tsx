...
import { ScrollArea, ScrollBar } from "@/registry/carbon/ui/scroll-area";
        ...
        >
          {examples.map((example, index) => (
            <Link
              ...
              key={example.href}
              className={cn(
                "flex h-7 items-center justify-center rounded-cb-md px-4 text-center text-sm transition-colors hover:bg-cb-accent/80 hover:text-cb-ring",
                pathname?.startsWith(example.href) ||
                  (index === 0 && pathname === "/")
                  ? "bg-cb-accent font-medium text-ring"
